...
public class SelectListServerFilterController implements Serializable {
  ...
  public List<SolarObject> getSolarObjectsOne() throws InterruptedException {
    if (queryOne == null || queryOne.isEmpty()) {
      return new ArrayList<>();
    } else {
      return solarObjectsOne.stream().filter(p -> StringUtils.containsIgnoreCase(p.getName(), queryOne)).toList();
    }
  }
  ...
  public String getSelectedSolarObjectsString() {
    ...
    if (selectedSolarObjects != null) {
      for (SolarObject selectedSolarObject : selectedSolarObjects) {
        stringBuilder.append(selectedSolarObject);
