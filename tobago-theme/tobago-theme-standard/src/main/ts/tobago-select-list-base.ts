...
Type: ${data.type}`);
  ...
  get delay(): number {
    const delay = this.hiddenQueryInput.getAttribute("delay");
    return delay ? parseInt(delay) : null;
  }

  get minChars(): number {
    const minChars = this.hiddenQueryInput.getAttribute("min-chars");
    return minChars ? parseInt(minChars) : null;
  }
