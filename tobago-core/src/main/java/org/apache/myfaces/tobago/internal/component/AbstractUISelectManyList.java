...
public abstract class AbstractUISelectManyList extends AbstractUISelectManyBase
  ...
  private transient Optional<UIComponent> abstractUIFilter = null; //set "null" to detect if initialized
  ...
  public AbstractUIFilter getAbstractUIFilter() {
    if (abstractUIFilter == null) {
      abstractUIFilter = getChildren().stream()
          .filter(AbstractUIFilter.class::isInstance)
          .findFirst();
    }
    return (AbstractUIFilter) abstractUIFilter.orElse(null);
  }
