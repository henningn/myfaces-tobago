...
public class SelectOneListRenderer<T extends AbstractUISelectOneList> extends SelectOneRendererBase<T> {
  ...
  private void encodeOptions(
    ...
    final TobagoResponseWriter writer = getResponseWriter(facesContext);
    final Boolean showFooter = component.getShowFooter();
    final Markup markup = component.getMarkup() != null ? component.getMarkup() : Markup.NULL;
    ...
    writer.startElement(HtmlElements.TR);
    writer.writeClassAttribute(
        TobagoClass.NO__ENTRIES,
        (showFooter == null && entryCount > 0) || (showFooter != null && !showFooter) ? BootstrapClass.D_NONE : null);
    writer.startElement(HtmlElements.TD);

    final UIComponent footer = ComponentUtils.getFacet(component, Facets.footer);
    if (footer != null) {
  ...
  private boolean isDisabled(T component, final AbstractUIFilter abstractUIFilter, List<SelectItem> items) {
    return (abstractUIFilter == null && !items.iterator().hasNext())
